...
public enum FakeSync {
    ...
    public static func run(root: URL) async throws -> [String] {
        ...
        // A new seam goes in a function, not in another two hundred lines here.
        // See `noteTagSeams`: this one has a hard size limit and it has been hit.
        out += try await noteTagSeams(mac: mac, phone: phone, macLib: macLib,
    ...
    /// Everything about a tag on a note: the digest, an edit that knows only title
    /// and body, a note lost locally, and the hand-written forms.
    ///
    /// **Split out of `run` as a build constraint rather than for taste.** That
    /// function is one long sequence by design, but it compiles with
    /// whole-module-optimization and the Swift frontend stops being able to
    /// hold it somewhere under two thousand lines. Measured: at 1,775 lines
    /// CI's Xcode 26.3 failed with `unable to open dependencies file
    /// (ListenKit-primary.d)`, which is what the driver reports when the
    /// frontend died before writing it, and the same file compiled without
    /// complaint on Xcode 26.6 on a developer machine. **So a local build
    /// cannot be the check for this**, and the only honest test is CI.
    ///
    /// Reverting exactly the 223 lines that had been added to `run` turned CI
    /// green, which is how the size was established rather than guessed.
    private static func noteTagSeams(
        mac: CloudSyncCore, phone: CloudSyncCore,
        macLib: Library, phoneLib: Library,
        store: MemoryStore, key: PairingKey, id: String
    ) async throws -> [String] {
        var out: [String] = []
        func ok(_ what: String) { out.append("  ok: \(what)") }
        func check(_ condition: Bool, _ what: String) throws {
    ...
    /// A note deleted on another device's say-so is recoverable, which was true of
    /// the pull path and not of the push one. Split out for the same reason.
    private static func noteTrashSeam(
        mac: CloudSyncCore, phone: CloudSyncCore,
        macLib: Library, phoneLib: Library,
        store: MemoryStore, key: PairingKey, id: String
    ) async throws -> [String] {
